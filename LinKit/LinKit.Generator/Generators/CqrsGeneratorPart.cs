...
internal static class CqrsGeneratorPart
{
    ...
    public static void Initialize(IncrementalGeneratorInitializationContext context)
    {
        ...
        IncrementalValuesProvider<INamedTypeSymbol> handlersFromAttribute = context.SyntaxProvider.ForAttributeWithMetadataName(HandlerAttributeName, (n, _) => n is ClassDeclarationSyntax, (c, _) => (INamedTypeSymbol)c.TargetSymbol).Where(s => s is not null);
        IncrementalValuesProvider<INamedTypeSymbol> handlersFromContext = context.SyntaxProvider.ForAttributeWithMetadataName(ContextAttributeName, (n, _) => n is ClassDeclarationSyntax, (c, _) => c).SelectMany((data, _) =>
        {
            ...
            if (attributeData is null || attributeData.ConstructorArguments.Length == 0) return ImmutableArray<INamedTypeSymbol>.Empty;
            var constructorArgs = attributeData.ConstructorArguments[0];
            if (constructorArgs.Kind != TypedConstantKind.Array) return ImmutableArray<INamedTypeSymbol>.Empty;
            foreach (var typeConstant in constructorArgs.Values)
            {
                if (typeConstant.Value is INamedTypeSymbol handlerTypeSymbol) handlers.Add(handlerTypeSymbol);
            }
            return handlers.ToImmutableArray();
        });
        var allHandlerSymbols = handlersFromAttribute.Collect().Combine(handlersFromContext.Collect());
        IncrementalValueProvider<IReadOnlyList<HandlerInfo>> collectedHandlers = allHandlerSymbols.Select((tuple, _) =>
        {
            var uniqueHandlers = new HashSet<INamedTypeSymbol>(SymbolEqualityComparer.Default);
            foreach (var handler in tuple.Left) uniqueHandlers.Add(handler);
            foreach (var handler in tuple.Right) uniqueHandlers.Add(handler);
        ...
        IncrementalValuesProvider<BehaviorInfo?> allBehaviors = context.SyntaxProvider
            ...
            transform: (c, _) =>
            {
                ...
                var targetInterfaceType = attributeData.ConstructorArguments[0].Value as INamedTypeSymbol;
                if (targetInterfaceType is null) return null;
                var order = attributeData.ConstructorArguments.Length > 1 ? (int)attributeData.ConstructorArguments[1].Value! : 0;
        ...
        context.RegisterSourceOutput(combined, (spc, source) =>
        {
            ...
            var extensionsSource = SourceGenerationHelper.GenerateMediatorExtensions(handlers, behaviors);
            spc.AddSource("Cqrs.Mediator.Extensions.g.cs", SourceText.From(extensionsSource, Encoding.UTF8));

            var diSource = SourceGenerationHelper.GenerateCqrsDI(handlers, behaviors);
            spc.AddSource("Cqrs.DependencyInjection.g.cs", SourceText.From(diSource, Encoding.UTF8));
        });
    }
    ...
    private static HandlerInfo? GetHandlerInfo(INamedTypeSymbol classSymbol)
    {
        ...
        var responseTypeSymbol = handlerInterface.TypeArguments.Length > 1
            ? handlerInterface.TypeArguments[1]
            : null;

        var responseTypeName = responseTypeSymbol?.ToDisplayString(SymbolDisplayFormat.FullyQualifiedFormat)
                               ?? "System.ValueTuple";

        var markerInterfaces = requestTypeSymbol.AllInterfaces
