...
public static class MapperGeneratorPart
{
    ...
    )
    {
        ...
        // --- LOGIC CHỌN CONSTRUCTOR MỚI: ƯU TIÊN PARAMETERLESS ---
        var parameterlessCtor = cfg.DestSymbol.Constructors.FirstOrDefault(c =>
        ...
        {
            // Nếu có constructor rỗng, dùng nó luôn để an toàn
            selectedCtor = parameterlessCtor;
        }
        else
        {
            // Nếu bắt buộc phải có tham số (ví dụ Record), tìm Constructor map được nhiều nhất
            ...
            foreach (var ctor in constructors)
            {
                ...
                {
                    var expr = rules.TryGetValue(p.Name, out var r)
                        ? r.SourceExpression
                        : TryAutoMap(p.Name, p.Type, srcProps, allMapPairs, null);
                    if (expr == null)
                    {
                        canMapAll = false;
                        break;
            ...
            // Fallback cuối cùng nếu ko tìm được cái nào khớp hoàn toàn (tránh lỗi compile)
            if (selectedCtor == null)
            {
                selectedCtor = constructors.FirstOrDefault();
                if (selectedCtor != null)
                {
                    foreach (var p in selectedCtor.Parameters)
                    {
                        var expr = rules.TryGetValue(p.Name, out var r)
                            ? r.SourceExpression
                            : TryAutoMap(p.Name, p.Type, srcProps, allMapPairs, null);
        ...
        {
            foreach (var p in selectedCtor.Parameters)
                mappedMembers.Add(p.Name);
        }

        // 2. Map tất cả Properties còn lại (bao gồm cả của class cha)
        foreach (var dp in GetSettableProps(cfg.DestSymbol))
        {
            // Nếu property đã được map qua constructor rồi thì thôi (trừ khi dùng ctor rỗng)
            ...
            if (expr != null)
            {
                if (dp.SetMethod?.IsInitOnly == true)
    ...
    private static string GenerateCode(List<MapperInfo> infos)
    {
        ...
        foreach (var nsGroup in infos.GroupBy(m => m.Namespace))
        {
            ...
            foreach (var m in nsGroup)
            {
                ...
                var allInit = m.InitOnlyAssignments.Concat(m.SettableAssignments).ToList();
                if (allInit.Any())
                {
                    sb.AppendLine($"\n{mi}        {{");
                    foreach (var a in allInit)
                        sb.AppendLine($"{mi}            {a.Name} = {a.Expr},");
                    sb.AppendLine($"{mi}        }};");
                }
                else
                    sb.AppendLine(";");
                ...
                foreach (var a in m.SettableAssignments)
                    sb.AppendLine($"{mi}        destination.{a.Name} = {a.Expr};");
                sb.AppendLine($"{mi}    }}");
    ...
    #region Parsing Logic (Bao gồm logic inheritance)
        ...
        switch (methodName)
        {
            ...
            case "MapFrom":
                ...
                )
                {
                    ...
                    CollectRequiredNamespaces(srcLambda.Body, model, ns);
                    return (
                        new ForMemberRule(
                            destName,
    ...
    private class LambdaBodyRewriter : CSharpSyntaxRewriter
    {
        ...
        public static string Translate(LambdaExpressionSyntax lambda, string newName)
        {
            var p = lambda switch
            {
                ...
                ParenthesizedLambdaExpressionSyntax px =>
                    px.ParameterList.Parameters.FirstOrDefault(),
                _ => null,
            };
            if (p == null)
                return lambda.Body.ToString();
            return new LambdaBodyRewriter(p.Identifier.ValueText, newName)
                .Visit(lambda.Body)
                .ToString();
        }
