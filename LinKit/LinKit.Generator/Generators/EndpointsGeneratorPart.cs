...
internal static class EndpointsGeneratorPart
{
    ...
    private static string GenerateEndpointsExtension(IReadOnlyList<EndpointInfo> endpoints)
    {
        ...
        foreach (var endpoint in endpoints)
        {
            ...
            bool isBodyMethod = endpoint.HttpMethod.Equals("POST", StringComparison.OrdinalIgnoreCase) ||
            ...
            if (isBodyMethod)
            {
                handlerParams.Add($"[Microsoft.AspNetCore.Mvc.FromBody] {endpoint.RequestType} request");
            }
            else
            {
                handlerParams.Add($"[Microsoft.AspNetCore.Http.AsParameters] {endpoint.RequestType} request");
            }
            ...
            var mapMethod = endpoint.HttpMethod switch
            {
                ...
                "Delete" => "MapDelete",
                "Patch" => "MapPatch",
                _ => $"MapMethods(\"{endpoint.Route}\", new[] {{ \"{endpoint.HttpMethod.ToUpper()}\" }})",
            };

            sb.AppendLine($"            app.{mapMethod}(\"{endpoint.Route}\", async ({string.Join(", ", handlerParams)}) =>");
            sb.AppendLine("            {");
            ...
            {
                sb.AppendLine($"                var result = await mediator.QueryAsync<{endpoint.ResponseType.TrimEnd('?')}> (request, cancellationToken);");
                sb.AppendLine("                return result is not null ? Results.Ok(result) : Results.NotFound();");
            }

            sb.AppendLine("            })");

            var endpointName = $"{endpoint.HttpMethod}{endpoint.RequestType.Split('.').Last()}{GetDeterministicHashCode(endpoint.Route)}";
            ...
            {
                sb.AppendLine($"            .Produces<{endpoint.ResponseType}>(StatusCodes.Status200OK)");
    ...
    private static string GenerateExceptionHandlerMiddleware()
    {
        ...
        sb.AppendLine("    {");
        sb.AppendLine("        public static IApplicationBuilder UseGeneratedExceptionHandler(this IApplicationBuilder app)");
        sb.AppendLine("        {");
        sb.AppendLine("            app.UseExceptionHandler(appError =>");
        sb.AppendLine("            {");
        sb.AppendLine("                appError.Run(async context =>");
        sb.AppendLine("                {");
        sb.AppendLine("                    var contextFeature = context.Features.Get<IExceptionHandlerFeature>();");
        sb.AppendLine("                    if (contextFeature != null)");
        sb.AppendLine("                    {");
        sb.AppendLine("                        var logger = context.RequestServices.GetService(typeof(ILogger<object>)) as ILogger;");
        sb.AppendLine("                        context.Response.ContentType = \"application/problem+json\";");
        sb.AppendLine();
        sb.AppendLine("                        switch (contextFeature.Error)");
        sb.AppendLine("                        {");
        sb.AppendLine("                            case ValidationException ex:");
        sb.AppendLine("                                context.Response.StatusCode = (int)HttpStatusCode.BadRequest;");
        sb.AppendLine("                                await context.Response.WriteAsJsonAsync(new { Title = \"Validation Error\", Detail = ex.Message, Status = 400 });");
        sb.AppendLine("                                break;");
        sb.AppendLine("                            default:");
        sb.AppendLine("                                logger?.LogError($\"Something went wrong: {contextFeature.Error}\");");
        sb.AppendLine("                                context.Response.StatusCode = (int)HttpStatusCode.InternalServerError;");
        sb.AppendLine("                                await context.Response.WriteAsJsonAsync(new { Title = \"An unexpected error occurred.\", Detail = \"Please try again later.\", Status = 500 });");
        sb.AppendLine("                                break;");
        sb.AppendLine("                        }");
        sb.AppendLine("                    }");
        sb.AppendLine("                });");
