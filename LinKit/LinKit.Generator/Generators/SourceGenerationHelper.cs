...
{
    public static string GenerateMediatorExtensions(
...
"
            ...
            foreach (var behavior in applicableBehaviors.AsEnumerable().Reverse())
            {
                ...
                sb.AppendLine(
                    $@"                next = () => mediator.Services.GetRequiredService<{closedBehaviorType}>().HandleAsync({requestParamName}, capturedNext, ct);"
                );
                sb.AppendLine("            }");
            }
...
"
    ...
    public static string GenerateCqrsDI(
        IReadOnlyList<HandlerInfo> handlers,
        IReadOnlyList<BehaviorInfo> availableBehaviors
    )
    {
