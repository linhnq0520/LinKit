                        ...
                        if (behaviors.Any())
                        {
                            ...
                            foreach (var behavior in behaviors)
                            {
                                if (
                                    ...
                                    && registeredBehaviors.Add(behavior.UnboundBehaviorType)
                                )
                                {
                                    services.Add(
                                        new CqrsServiceInfo(
...
namespace LinKit.Generated.Cqrs
{
    internal sealed class Mediator : IMediator
    {
        ...
        foreach (
            var handler in handlers.Where(h =>
                h.HandlerInterface.Contains(ICommandHandlerName)
                && h.ResponseType != "System.ValueTuple"
            )
        )
        {
            sb.AppendLine(
                $"                {handler.RequestType} c => (Task<TResult>)(object)HandleResultRequest(c, cancellationToken),"
            );
        }
        sb.AppendLine(
            @"                _ => throw new InvalidOperationException($""No handler found for command type {command.GetType().FullName}"")
        ...
        {
            sb.AppendLine(
                $"                {handler.RequestType} q => (Task<TResult>)(object)HandleResultRequest(q, cancellationToken),"
            );
        }
        sb.AppendLine(
